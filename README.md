...
This monorepo implements a lightweight event-driven microservices architecture for managing e-commerce order processing — from API Gateway and Order handling to asynchronous Inventory, Payment, and Reward services through RabbitMQ:
...
```
           ...
           +---------------------------------------+
           |       🌐 API Gateway + Orders         |
           |     (NestJS - same Render service)    |
           +---------------------------------------+
                       |             |
                       | 2. Save     | 3. Emit event "order_created"
                       |   Order     v
                       |     +--------------------------+
                       |     |   🐇 RabbitMQ Broker     |
                       |     | (CloudAMQP Free Tier)    |
                       ...
                       |                 |
                       |                 |
                       |   4a Deliver    |   4b Deliver    |   4c Deliver
                       v                 v                 v
     +-----------------------+  +----------------------+  +----------------------+
     | 🏭 Inventory Service   |  | 💳 Payment Service  |  | 🎁 Reward Service   |
     | (Render free service)  |  | (Render free svc)   |  | (Render free svc)   |
     +-----------+------------+  +----------+-----------+  +----------+----------+
                 |                          |                          |
   Process & update stock      Process & update payment      Process & update rewards
                 |                          |                          |
                 v                          v                          v
     +-------------------+     +-------------------+     +-------------------+
     |  Inventory  DB    |     |   Payment  DB     |     |   Rewards  DB     |
     +-------------------+     +-------------------+     +-------------------+
```
https://api.cloudamqp.com/console/186fada6-9514-482a-88e7-e7fa87e4ae0e/details

### DB

```
CREATE DATABASE adidas_order_prod;
CREATE DATABASE adidas_inventory_prod;
CREATE DATABASE adidas_payment_prod;
CREATE DATABASE adidas_reward_prod;
```
